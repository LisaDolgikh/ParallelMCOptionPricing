...
TEST(BlackScholesTest, CallOptionValue) {
    auto res = mcopt::BlackScholesAnalytical::calculate(100.0, 100.0, 1.0, 0.05, 0.2,
                                                        mcopt::OptionType::Call);
...
TEST(BlackScholesTest, PutCallParityAnalytical) {
    ...
    auto putRes =
        mcopt::BlackScholesAnalytical::calculate(S0, K, T, r, sigma, mcopt::OptionType::Put);
